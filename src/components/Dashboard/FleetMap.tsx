...
import { Device, Vehicle, MapProvider } from '../../types';
import { useGoogleFleetMap } from '../../hooks/useGoogleFleetMap';
import { OpenStreetMapFleetMap } from './OpenStreetMapFleetMap';
...
interface GoogleFleetMapProps extends FleetMapProps {
  apiKey: string;
}

const GoogleFleetMap: React.FC<GoogleFleetMapProps> = ({
  apiKey,
  devices,
  vehicles = [],
  selectedDevice,
  ...
  const {
    ...
    focusOnDevice,
    loadError
  } = useGoogleFleetMap({ apiKey, devices, vehicles });
...
export const FleetMap: React.FC<FleetMapProps> = ({ devices, vehicles = [], selectedDevice, onDeviceSelect }) => {
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        ...
        <div className="relative h-72 sm:h-80 lg:h-96 bg-gradient-to-br from-blue-50 to-green-50 flex items-center justify-center">
          <div className="text-center space-y-3 px-4">
            <MapPin className="w-12 h-12 sm:w-16 sm:h-16 text-gray-400 mx-auto" />
            <p className="text-sm sm:text-base text-gray-600 font-medium">Configure a API do Google Maps</p>
            <p className="text-xs sm:text-sm text-gray-500 max-w-xs mx-auto">
              Para visualizar o mapa interativo do Google, acesse Administração → Configurações → Mapas, informe sua chave de API e habilite o faturamento.
            </p>
            <p className="text-xs sm:text-sm text-blue-600 max-w-xs mx-auto flex items-center gap-2 justify-center">
              <MapIcon size={16} />
              Precisa de um mapa gratuito agora? Troque o provedor para OpenStreetMap nas configurações.
            </p>
          </div>
        </div>
